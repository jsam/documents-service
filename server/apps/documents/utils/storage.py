...
def get_step2_detections_path(job_id: UUID) -> str:
    return f'{get_job_base_path(job_id)}/ml_inference'
...
def get_step6_ocr_path(job_id: UUID) -> str:
    return f'{get_job_base_path(job_id)}/ocr_processing'


def get_ocr_result_path(job_id: UUID, page_number: int) -> str:
    return f'{get_step6_ocr_path(job_id)}/page_{page_number:04d}_ocr.json'
